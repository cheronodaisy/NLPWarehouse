    ...
    with open(input_file, mode='r', newline='') as file:
        ...
        for index, row in enumerate(reader, start=1):
            if row:  # Ensure the row is not empty
                ...
                output_filename = "/home/dereje_senbatu/10x-project/NLPWarehouse/data/bbc/afla/afla"  # Use a single filename here
                write_to_csv(scraped_data, output_filename, mode='a')
                # Add a delay between requests to avoid overwhelming the server
                time.sleep(3)

# Path to the input CSV file containing the sub URLs
input_file = "/home/dereje_senbatu/10x-project/NLPWarehouse/utils/bbc/afla/sub_urls.csv"
